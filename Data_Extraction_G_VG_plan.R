...
path <- "2022_07_04_IllerHenry/LongitudinalSections/corrected_plan"
...
dataset %>%  # replace file names with type and add velocities column
  ...
  mutate(discharge = case_when(
    grepl("5.0", type, fixed = T) ~ 5.0,
    grepl("_8.0", type, fixed = T) ~ 8.0,
    grepl("11.0", type, fixed = T) ~ 11.0,
    grepl("14.0", type, fixed = T) ~ 14.0,
    grepl("20.0", type, fixed = T) ~ 20.0
...
full_lh$life.history <- c(rep("Larval",488),
                          rep("Juvenile",488),
                          rep("Summer",488),
                          rep("Winter",488),
                          rep("Spawning",488))
...
write.csv(lhd_flush, "Tables/flushed_plan.csv")
